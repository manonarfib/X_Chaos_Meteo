...
# To numpy array
data_train = f["/train/tp_images"][:100, :,
                                   :96, :96]  # remove the slicing later
max_train = 0.024250908
data_train = data_train/max_train  # normalize data


data_train1 = f["/train/u100_images"][:100,
                                      :, :96, :96]  # load wind u component
data_train2 = f["/train/v100_images"][:100,
                                      :, :96, :96]  # load wind v component
# pythagorean theorem to get wind speed
...
# Instantiation
model = Late_WF_UNet(lags, lat, long, feats, feats_out,
                     convFilters, dropoutRate)
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5,
                              patience=10, min_lr=0.0001, min_delta=0.00001, verbose=1)
...
training_generator = DataGenerator(
    [data_train, data_wind_speed_train], batch_size, lags, multistream=True)
validation_generator = DataGenerator(
    [data_val, data_wind_speed_val], batch_size, lags, multistream=True)
