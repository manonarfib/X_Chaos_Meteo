...
class WeatherCBM(nn.Module):
    """
    ConvLSTM model to predict a precipitations map (tp_6h)
    using a sequence of inputs, with an interpretable concepts bottleneck.
    ...
    Pipeline :
        ConvLSTM encoder
        -> projection to K concepts α_k
        -> linear combination Σ α_k B_k
        -> rain map
    """
    ...
    def __init__(
        ...
        C_last = hidden_channels[-1]

        # Projection in the concepts (α_k) space
        self.concept_projection = nn.Conv2d(
            in_channels=C_last,
            out_channels=n_concepts,
            kernel_size=1,
        )

        # Linear layer to learn B_k
        self.linear_combination = nn.Conv2d(
            in_channels=n_concepts,
            out_channels=1,
            kernel_size=1,
            bias=False,
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #projection in the concepts space
        alpha = torch.relu(self.concept_projection(h_T)) # (B, K, H, W)
